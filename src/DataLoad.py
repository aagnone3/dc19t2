...
class DataLoadDf(Dataset):
    ...
    def __init__(self, df, get_feature_file_func, encode_function, transform=None,
        ...
        self.filenames = df.feature_filename.drop_duplicates()
...
class PCEN:
    ...
    def __init__(self, delta=10, r=0.5, T=0.060, Q=50, tau=0.015, eps=1e-6, alpha=0.8):
        self.delta = delta
        self.r = r
        self.T = T
        self.Q = Q
        ...
        self.alpha = alpha
        self.pcen_kwargs = {
            # TODO need to estimate T empirically
            'time_constant': T,  # longer than time for event to change frequency bins
            'sr': 44100,
            'hop_length': 511,
            'gain': alpha,  # gain factor. alpha -> 1 cancels more background noise (and potentially also events)
            'power': r,  # exponent of the compression
            'bias': delta,  # bias point of the DRC
        }
    ...
    def __call__(self, sample):
        ...
        # axis argument should be index of time axis --> axis=0 since features are TxF
        sample[0] = pcen(sample[0], axis=0, **self.pcen_kwargs)
...
class AugmentGaussianNoise_T:
    ...
    def __init__(self, mean=0, std=0.5, as_tensor=True):
        self.mean = mean
        self.std = std
        self.as_tensor = as_tensor
        if as_tensor:
            self.f = self.f_tensor
            self.noise = distributions.Normal(float(self.mean), float(self.std))
        else:
            self.f = self.f_vector
    ...
    def f_vector(self, sample):
        sample, label = sample
        # TODO adding to sample?
        noise = sample + np.abs(np.random.normal(0, 0.5 ** 2, sample.shape))
        return sample, noise, label

    def f_tensor(self, sample):
        sample, label = sample
        noise = sample + torch.abs(self.noise.sample(sample.shape).cuda())
        return sample, noise, label
